...
->schema([
    ...
    ->schema([
        Select::make('shareholdable_type')
            ...
            ->options([
                ...
                \App\Models\Entity::class => 'Entity',
            ])
            ->required()
            ->reactive(),
        ...
        Select::make('shareholdable_id')
            ...
            ->options(function (callable $get) {
                ...
                if ($type === \App\Models\Entity::class) {
                    return \App\Models\Entity::pluck('name', 'id');
                }
    ...
    ->default(fn (callable $get, ?Entity $record): ?array => optional($record?->shareholders)->map(fn ($s) => [
        'shareholdable_type' => $s->shareholdable_type,
        'shareholdable_id' => $s->shareholdable_id,
...
public static function afterSave(Model $record): void
{
    ...
    DB::table('shareholders')->where('entity_id', $record->id)->delete();

    // Insertamos los nuevos accionistas
    foreach ($shareholders as $shareholder) {
        DB::table('shareholders')->insert([
            'entity_id' => $record->id,
            'shareholdable_type' => $shareholder['shareholdable_type'],
            'shareholdable_id' => $shareholder['shareholdable_id'],
            'percentage' => $shareholder['percentage'] ?? null,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }
...
{
    $data = $operation->getForm()->getState();

    // Clear existing
    $record->shareholders()->delete();

    // Sync new ones
    self::syncShareholders($record, $data['shareholders'] ?? []);
}
